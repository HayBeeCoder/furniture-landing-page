...
const Header = () => {
    ...
    const NavSlide = (isOpen: boolean) =>  isOpen ? " translate-x-full delay-150" : " translate-x-0 delay-150 "
    const OverlaySlide = (isOpen: boolean) =>  isOpen ? " translate-x-full " : " translate-x-0  delay-150"
    ...
    return (
        <header className='font-montserrat px-4 md:px-8 lg:px-12 py-5 flex justify-between items-end'>
            ...
            <nav className={'fixed z-50 flex flex-col h-screen md:h-max md:w-max top-0 w-4/5 items-center justify-center  right-0 md:relative transform transition-transform ease-in bg-white md:transform-none  md:gap-2 md:justify-between md:flex-row text-left rounded-t-md rounded-b-md ' + NavSlide(open)}>
